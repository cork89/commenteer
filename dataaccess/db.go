...
FROM links l
WHERE l.cdn_image_url != '' ORDER BY l.created_date DESC LIMIT 3 OFFSET ($1);`, offset)
...
WHERE l.cdn_image_url != '' AND l.user_id = u.user_id AND u.user_id = ($1)
 ORDER BY l.created_date DESC LIMIT 3 OFFSET ($2);`, userId, offset)
	// return handleRetrieve(rows, err)
	if err != nil {
		log.Printf("Error retrieving recent links, %v", err)
		return nil, true
...
ORDER BY l.created_date DESC LIMIT 3 OFFSET ($3);`, userId, username, offset)
